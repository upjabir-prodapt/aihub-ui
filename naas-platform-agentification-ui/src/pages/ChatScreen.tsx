...
import UnifiedLeftPanel from '../registry/UnifiedLeftPanel';
...
// "I'll check circuit #N..." messages (see registry/chatExtras.tsx's
// callers). `labelKey` is only what the button displays.
...
// "/" — a single, unified interface: a persistent Navbar on top (brand +
// which specialist is currently active + language/theme/admin), and below
// it either a clean, centered "what do you want to do?" welcome screen
// (Claude-style, no agent cards — the orchestrator decides which
// specialist handles a request, the user never picks one directly) or,
// once the conversation has actually started, the same split left-panel/
// chat layout AgentScreen.tsx's standalone screens use. The 3 specialists
// remain directly reachable at their own /agents/:agentId route
// (AgentScreen.tsx, untouched) for isolated testing — just no longer
// linked from this screen.
//
// One ChatPanel instance carries a session across the welcome <-> split
// layout switch — only its `variant` prop and surrounding layout change
// (see ChatPanel.css's `chat-panel--welcome`), so the session and its
// accumulated messages carry over untouched. autoGreet is off here (unlike
// AgentScreen.tsx's standalone screens) since the welcome heading below
// already covers what an opening greeting bubble would say — the first
// message actually shown once the transcript appears is the user's own.
// This instance is only ever remounted (a fresh session, via the
// `chatInstance` key) when the user explicitly closes the session with
// Navbar's "New chat" button — see handleNewChat.
export default function ChatScreen() {
  ...
  // as the user picked one. See registry/leftPanels.tsx's LeftPanelProps.
  ...
  // session — ChatPanel's onFirstMessageSent, fired on Enter/Send (or a
  // sample prompt's own send, see sendSamplePrompt below), deliberately
  ...
  // Bumped on "New chat" (see handleNewChat) and used as ChatPanel's `key`
  // — changing a component's key forces React to unmount the old instance
  // and mount a fresh one, which is what actually closes the old session
  // (a new sessionId, empty messages) rather than just hiding it.
  ...
  function sendSamplePrompt(text: string) {
    // No explicit setChatStarted(true) here — ChatPanel's send() (which
    // sendMessage below calls into) fires onFirstMessageSent itself the
    // same way a typed Enter/Send does, so this stays a single code path
    // for "the session's first message was sent" regardless of how.
  ...
  return (
    <div className="chat-screen-page">
      <Navbar activeAgent={lastActiveAgent} chatStarted={chatStarted} onNewChat={handleNewChat} />
      <div

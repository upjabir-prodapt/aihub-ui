...
import { timeAgo, dayBucket } from '../../shared/utils/time';
...
const JobTrackerPage: React.FC = () => {
  // A service page's "All jobs" link navigates to /tracker?service=<name>, so
  // the pre-filter now survives a reload and can be shared as a URL.
  const [searchParams] = useSearchParams();
  const initialServiceFilter = readServiceParam(searchParams.get('service'));
  // Entitlements replace the old per-service `canTranslation` flags: there is
  // one session now, and what varies is which Entra roles it carries.
  const { translation: canTranslation, sales: canSales } = useEntitlements();
  ...
  const loadHistory = useCallback(async () => {
    ...
    const translationReq: Promise<LegacyJobStatusResponse[]> = canTranslation
      ? translationApi.listJobs()
      : Promise.resolve([]);
    const salesReq: Promise<ResearchJobListItem[]> = canSales
      ? listResearchJobs()
      : Promise.resolve([]);
